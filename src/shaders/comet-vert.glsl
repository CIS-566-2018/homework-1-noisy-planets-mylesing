...
void main()
{
    ...
    // cloud noise
    float n1 = snoise(fs_Nor.xyz * 1.5 + u_Time * 0.01);
    float n2 = noise(fs_Nor.xyz, 5, 5.8, 0.75) * 0.015 - 0.01;
    float n3 = snoise(fs_Nor.xyz * 2.0 + u_Time * 0.08);
    float n = n1 * 0.5 + n2 + n3;
    vec3 offset = fs_Pos.xyz + (fs_Nor.xyz * n * 0.25);
    ...
    vec2 offset2 = vec2(snoise(fs_Pos.xyz + vec3(offset1, 1.0) + vec3(1.7, 9.2, 2.5)), 
                        snoise(fs_Pos.xyz + sin(u_Time * 0.01) + vec3(offset1, 1.0) + vec3(8.3, 2.8, 4.5)));
    float perlin = snoise(fs_Pos.xyz + vec3(offset1 * 2.0, 1.0)) + n;
    ...
    offset = fs_Pos.xyz + (fs_Nor.xyz * perlin * 0.05 + threshold * 10.0 * u_Time);

    //float zOffset = fs_Pos.z + fs_Nor.z * snoise(fs_Pos.xyz + mod(u_Time * 0.05, 1000.0) * 0.05) * 0.5;
